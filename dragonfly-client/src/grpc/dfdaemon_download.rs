...
use dragonfly_client_util::{
    ...
    http::{fast_path_metadata, hashmap_to_headermap, headermap_to_hashmap, parse_range_header},
...
impl DfdaemonDownload for DfdaemonDownloadServerHandler {
    ...
    async fn download_task(
        ...
        // ── Signed-range fast-path gate (gRPC download path) ─────────────────
        ...
        let known_metadata = {
            let fp = fast_path_metadata(
                &download.request_header,
                &download.url,
                download.piece_length,
            );
            // Strip X-Dragonfly-Content-Length before forwarding to the origin
            // (case-insensitive — HashMap keys are lowercased by headermap_to_hashmap).
            download
                .request_header
                .retain(|k, _| !k.eq_ignore_ascii_case("X-Dragonfly-Content-Length"));
            fp.map(|fp| {
                info!(
                    "fast-path (gRPC): range bytes={}-{} content_length={} piece_length={}",
